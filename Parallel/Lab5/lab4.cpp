...
double doubleIntegrate(double a, double b, double c, double d, int n) {
    ...
    return ((d - c)/(8 * n))*(integrate(a, b, y, n) + integrate(a, b, y, n) + result);
}
...
int main(int argc, char** argv) {
    ...
    local_result = doubleIntegrate(a + (rank * (b - a) / size), a + ((rank + 1) * (b - a) / size), c + (rank * (d - c) / size), c + ((rank + 1) * (d - c) / size), local_n);
